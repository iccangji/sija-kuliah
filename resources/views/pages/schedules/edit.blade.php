  ...
  <div class="daily-schedule w-100">
    ...
    <div class="container mt-4">
      <form action="" method="post">
        @csrf
        <input name="id" type="text" class="form-control d-none" id="floatingInput" placeholder="09:30" value="{{$schedules['id']}}" required>
        ...
        <select name="course" class="form-select mb-3" aria-label="Default select example" style="font-family: inherit" required>
          ...
          @foreach ($courses as $course)
            <option value="{{$course['kode']}}" @if ($course['kode'] == $schedules['kode'])
                selected
            @endif>{{$course['nama']}}</option>  
          @endforeach
        </select>
        <label for="room">Ruangan</label>
        <select name="room" class="form-select mb-3" aria-label="Default select example" style="font-family: inherit" required>
          <option selected>Pilih Ruangan</option>
          <option value="IT1" @if ($schedules['ruang'] == 'IT1')
          selected
          @endif>IT-1</option>  
          <option value="IT2" @if ($schedules['ruang'] == 'IT2')
          selected
          @endif>IT-2</option>  
          <option value="IT3" @if ($schedules['ruang'] == 'IT3')
          selected
          @endif>IT-3</option>  
          <option value="LAB. RPL" @if ($schedules['ruang'] == 'LAB. RPL')
          selected
          @endif>LAB. RPL</option>  
          <option value="LAB. SI" @if ($schedules['ruang'] == 'LAB. SI')
          selected
          @endif>LAB. SI</option>  
          <option value="LAB. MULTIMEDIA" @if ($schedules['ruang'] == 'LAB. MULTIMEDIA')
          selected
          @endif>LAB. MULTIMEDIA</option>
        </select>
        <label for="day">Hari</label>
        <select name="day" class="form-select mb-3" aria-label="Default select example" style="font-family: inherit" required>
          <option selected>Pilih Hari</option>
          <option value="1" @if ($schedules['hari'] == 1)
          @endif
          selected>
          Senin</option>  
          <option value="2" @if ($schedules['hari'] == 2)
          @endif
          selected>
          Selasa</option>  
          <option value="3" @if ($schedules['hari'] == 3)
          @endif
          selected>
          Rabu</option>  
          <option value="4" @if ($schedules['hari'] == 4)
          @endif
          selected>
          Kamis</option>  
          <option value="5" @if ($schedules['hari'] == 5)
          @endif
          selected>
          Jumat</option>
        </select>
        <div class="row">
          <div class="col-6">
            <div class="form-floating mb-3">
              <input name="time-start" type="time" class="form-control" id="floatingInput" placeholder="09:30" value="{{str_replace('.',':',substr($schedules['jam'], 0, 5))}}" required>
              <label for="floatingInput">Jam Masuk</label>
            </div>
          </div>
          <div class="col-6">
            <div class="form-floating mb-3">
              <input name="time-end" type="time" class="form-control" id="floatingInput" placeholder="09:30" value="{{ str_replace('.',':',substr($schedules['jam'], 6))}}" required>
              <label for="floatingInput">Jam Keluar</label>
            </div>
          </div>
        </div>
        <div class="form-floating mb-3">
          <input name="students" value="{{$schedules['mahasiswa']}}" type="text" class="form-control" id="floatingInput" placeholder="Mahasiswa" required>
          <label for="floatingInput">Angkatan / Semester / Jumlah</label>
        </div>
        <div class="form-floating mb-3">
          <input name="class" value="{{$schedules['kelas']}}" type="text" class="form-control" id="floatingInput" placeholder="Mahasiswa" required>
        ...
        <select name="lecturer1" class="form-select mb-3" aria-label="Default select example" style="font-family: inherit" required>
          ...
          @foreach ($lecturers as $lecturer)
            <option value="{{$lecturer['nidn']}}" @if ($lecturer['nidn'] == $schedules['dosen1'])
                selected
            @endif>{{$lecturer['nama']}}</option>  
          @endforeach
        </select>
        ...
        <select name="lecturer2" class="form-select mb-3" aria-label="Default select example" style="font-family: inherit">
          ...
          @foreach ($lecturers as $lecturer)
            <option value="{{$lecturer['nidn']}}" @if ($lecturer['nidn'] == $schedules['dosen2'])
            selected
        @endif>{{$lecturer['nama']}}</option>  
          @endforeach
        </select>
        ...
        <select name="lecturer3" class="form-select mb-3" aria-label="Default select example" style="font-family: inherit">
          ...
          @foreach ($lecturers as $lecturer)
            <option value="{{$lecturer['nidn']}}" @if ($lecturer['nidn'] == $schedules['dosen3'])
            selected
        @endif>{{$lecturer['nama']}}</option>  
          @endforeach
        </select>
        <button class="btn btn-primary w-100 mt-2">Ubah Jadwal</button>
      </form>
    </div>
  </div>
</div>
